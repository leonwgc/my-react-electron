...
Array.prototype.twist = function (curr) {
  const included = this.includes(curr);

  return included ? this.filter((item) => item !== curr) : this.concat(curr);
};
...
export const clearUndefiend = (data) => {
  ...
  if (isArray) {
    return data.filter((item) => item !== undefined).map(clearUndefiend);
  }
  ...
  return keys.reduce((a, key) => {
    ...
    return Object.assign({}, a, {
      [key]: clearUndefiend(value),
    });
  }, {});
};

export const withClassName = (className) => (Comp) =>
  forwardRef((props = {}, ref) => {
    const { className: propsClassName, ...others } = props;
    ...
    return <Comp className={cls} {...others} />;
  });
